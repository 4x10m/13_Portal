...
export async function POST(
  ...
  try {
    ...
    if (totalUnchecked > 0) {
      ...
      const milestoneTitle = `📋 Tâches importées (${new Date().toLocaleDateString("fr-FR")})`;
    ...
    return NextResponse.json({
      ...
      message: `${imported} tâche${imported > 1 ? "s" : ""} importée${imported > 1 ? "s" : ""} depuis ${allResults.length} fichier${allResults.length > 1 ? "s" : ""} (${skippedChecked} déjà cochée${skippedChecked > 1 ? "s" : ""} ignorée${skippedChecked > 1 ? "s" : ""})`,
    });
  } catch (error) {
